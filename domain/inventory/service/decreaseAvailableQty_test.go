...
func Test_Service_DecreaseAvailableQty_Positive(t *testing.T) {
	var (
		id        uint64  = 1
		qty       float64 = 2
	...
	t.Run("when success", func(t *testing.T) {
		...
		err := m.service.DecreaseAvailableQty(m.Ctx, id, qty)
		assert.Nil(t, err)
	})
}

func Test_Service_DecreaseAvailableQty_Negative(t *testing.T) {
	var (
		id        uint64  = 1
		qty       float64 = 2
		mapUpdate         = map[string]interface{}{
	...
	t.Run("when failed on repository.DecreaseAvailableQty error", func(t *testing.T) {
		m := setupMock(t)
		defer m.Ctrl.Finish()
